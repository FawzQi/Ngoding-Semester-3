...
class Boss_Dog : public Character {
   public:
    Vector2 pos = {725, 525};
    float hp = 1000;
    int frame;
    ...
    void isGetHit() {
        ...
        if (player.attack_state && (player.img_frame == 5 || player.img_frame == 8) && (hit_reset + 10) <= game_frame) {
        ...
        for (bullet& bullets : dogs_bullet) {
            if (bullets.active) {
                ...
                if (CheckCollisionCircleRec({(this->direction ? 0 : -55) + bullets.pos.x + 385, bullets.pos.y + 195}, 30, player.hitBox())) {
                    player.hp -= 38;
                    player.stamina -= 7;
                    player.pos.x -= (this->direction ? -30 : 30);
        ...
        if (attack_state) {
            if (special_attack) {
                ...
                if (game_frame - frame >= 7) {
                    ...
                    if (img_frame == 8) img_frame = 5;

                    if (att_frame == 3 || att_frame == 8 || att_frame == 13) {
                        (!direction ? MakeBullet(dogs_bullet, {pos.x - 215, pos.y + 40}, {-35, 0}) : MakeBullet(dogs_bullet, {pos.x + 215, pos.y + 40}, {35, 0}));
                        if (pos.x + 460 > 0 && pos.x + 460 < 1920) {
                            direction ? pos.x += -50 : pos.x += 50;
                        }
                    }
                    frame = game_frame;
                }
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        // // gambar nilai test_pos dan test_pos2 440 570  570 1015
        ...
        //     break;
        // }
        //  DrawTexturePro(boss_dog.img[8], {0, 0, (float)boss_dog.img[8].width, (float)boss_dog.img[8].height}, {test_pos.x, test_pos.y, (float)boss_dog.img[8].width, (float)boss_dog.img[8].height}, {0, 0}, 0, WHITE);
        EndDrawing();
        game_frame++;
    }
